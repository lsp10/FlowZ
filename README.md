...
支持 NaïveProxy、VLESS、VMess、Trojan、Shadowsocks、Hysteria2、AnyTLS、TUIC、SOCKS、HTTP 协议。  
...
| **仅本地代理模式** | 取决于用户 DNS 配置中的 `enableFakeIp` 开关 |
...
FlowZ 的 DNS 配置采用**国内外分离**策略，通过两个独立的 DNS 服务器分别解析国内和海外域名，兼顾速度与安全。

### 配置项说明

| 配置项 | 默认值 | 用途 |
|---|---|---|
| **国内 DNS（直连）** | `https://doh.pub/dns-query` | 解析国内域名，直连发送，不经过代理 |
| **海外 DNS（代理）** | `https://dns.google/dns-query` | 解析海外域名，通过代理服务器发送，防止 DNS 污染 |
| **启用 FakeIP** | 关闭 | 启用后所有域名返回假 IP，不实际查询上游 DNS |

### 工作原理

FlowZ 启动代理时，会根据用户配置生成 sing-box 的 DNS 模块，包含以下 DNS 服务器：

| 内部名称 | 上游地址 | 连接方式 | 职责 |
|---|---|---|---|
| `dns-bootstrap-udp` | `223.5.5.5`（阿里 DNS） | UDP 直连 | 引导解析，解析 DoH 服务器自身的域名（如 doh.pub） |
| `dns-bootstrap` | `https://223.5.5.5/dns-query` | HTTPS 直连 | 引导解析的加密版本 |
| `dns-domestic` | 用户配置的国内 DNS | HTTPS 直连 | 解析命中 geosite-cn 的国内域名 |
| `dns-remote` | 用户配置的海外 DNS | HTTPS **经代理** | 解析海外域名，流量通过选中的代理服务器转发 |
| `dns-local` | 系统 DNS | 直连 | 兜底 |

### DNS 路由规则

sing-box 收到 DNS 查询后，按以下规则决定使用哪个 DNS 服务器：

```
① DoH 服务器域名（doh.pub、dns.google 等）→ dns-bootstrap-udp（防止解析死循环）
② geosite-cn 域名 → dns-domestic（国内 DNS 直连解析）
③ 其他域名 → dns-remote（海外 DNS 经代理解析）
```

### 为什么海外 DNS 必须走代理

```
用户 → [代理服务器] → dns.google → 返回真实 IP
```

如果海外 DNS 直连发送，GFW 会：
1. **拦截请求** — 部分 DoH 域名被封锁
2. **污染响应** — 返回虚假 IP，导致后续连接失败
3. **暴露意图** — ISP 能看到你在查询哪些海外域名

通过代理发送 DNS 请求，查询在境外完成，完全绕过 GFW 干扰。

### 为什么国内 DNS 走直连

```
用户 → doh.pub → 返回真实 IP → 直连访问
```

国内域名不需要代理，直连查询更快（延迟 10~30ms vs 代理 100~300ms），且国内 DoH 服务（如腾讯 DNSPod 的 doh.pub）不存在污染问题。

### FakeIP 模式下的 DNS 行为

启用 FakeIP 后，DNS 路由规则变为：

```
所有 A/AAAA 查询 → 直接返回 198.18.x.x 假 IP（不查询任何上游 DNS）
```

此时国内/海外 DNS 配置**仅在以下场景使用**：
- 路由引擎判定流量走直连时，direct 出站阶段重新解析真实 IP（使用 dns-domestic）
- 绕过 FakeIP 的域名（如银行域名）使用对应的 DNS 服务器解析

### 推荐配置

| 场景 | 国内 DNS | 海外 DNS |
|---|---|---|
| 默认（推荐） | `https://doh.pub/dns-query` | `https://dns.google/dns-query` |
| 追求隐私 | `https://doh.pub/dns-query` | `https://cloudflare-dns.com/dns-query` |
| 企业内网 | 内网 DNS 地址 | `https://dns.google/dns-query` |

### 代码实现位置

- 类型定义：`src/shared/types.ts` → `DnsConfig` 接口
- 默认值：`src/main/services/ConfigManager.ts` → `dnsConfig` 字段
- DNS 配置生成：`src/main/services/ProxyManager.ts` → `generateDnsConfig()` 方法
- DNS 规则生成：`src/main/services/ProxyManager.ts` → DNS rules 部分（根据代理模式和 FakeIP 开关动态生成）

---

## 📱 应用分流详解

应用分流可以为特定应用（如 YouTube、Telegram、币安等）单独指定流量走向，不受全局代理模式的影响。这是一个成熟稳定的功能，支持进程名精准匹配和域名/IP规则匹配。
...
| `processNames` | 进程名列表，按操作系统进程名匹配。如 `["Telegram", "Telegram.exe"]`。**仅 TUN 模式生效**，系统代理和仅本地代理模式下 sing-box 无法获取进程信息 |
...
**实际效果**：TUN 模式下，进程名规则先命中，覆盖面更广（进程的所有流量，包括非标准域名）；如果进程名没命中（系统代理或仅本地代理模式，或进程名不在列表中），域名/IP 规则兜底。
...
| 策略 | 含义 |
|---|---|
| **自动（跟随全局规则）** | 不单独设置，走全局兜底。智能分流模式下国内直连、国外走代理；全局代理模式下全部走代理 |
| **代理（主节点）** | 强制走首页当前选中的主节点，不受全局模式影响 |
| **独立路由节点** | 强制走某个**特定**节点，而不是主节点。适合需要为某个应用单独指定出口的场景 |
| **直连** | 强制直连，不经过任何代理 |
| **屏蔽** | 拦截该应用的所有流量 |

**"代理（主节点）"和"独立路由节点"的区别：**

- **代理（主节点）**：走首页选中的那个节点。主节点换了，这个应用也跟着换。
- **独立路由节点**：绕过主节点，单独指定一个固定节点。主节点怎么换都不影响这个应用。

**典型场景**：

你有 JP节点（日常使用）和 固定IP节点（需要固定出口）：
- 首页主节点选 JP节点
- ChatGPT 应用分流选"独立路由节点 → 固定IP节点"
- 效果：ChatGPT 走固定IP，其他流量走JP节点
...
| `enabled` | 是否启用这条规则（`false` 等同于"自动跟随全局"） |
| `targetServerId` | 指定代理服务器（仅 action 为 proxy 时有效），不填则走默认选中的主节点 |
...
| 部分 | 作用 |
|------|------|
| `rm -rf /Applications/FlowZ.app` | 删除 Applications 中的旧版本（`-rf` 强制递归删除 .app 目录） |
| `cp -R /Users/shipengliu/flowz/FlowZ/dist-package/mac/FlowZ.app /Applications/` | 将打包产物完整复制到 Applications（`-R` 递归复制目录） |
| `open /Applications/FlowZ.app` | 启动新安装的应用 |

三条命令用 `&&` 连接，前一条成功才执行下一条，确保不会在删除失败时覆盖或在复制失败时启动旧版。
...
FlowZ 在窗口隐藏到托盘后，10 分钟无操作会自动进入轻量模式，最大限度降低后台资源占用，同时保持代理服务正常运行。

### 触发与恢复

| 事件 | 行为 |
|------|------|
| 窗口失焦 / 隐藏后 10 分钟无操作 | 自动进入轻量模式 |
| 点击托盘图标重新打开窗口 | 自动退出轻量模式，恢复正常 |
| 用户手动操作 | 可在设置中关闭自动轻量模式 |

### 轻量模式下的资源优化

**主进程**
- 注销所有 IPC handlers（窗口不存在时无需响应渲染进程请求）
- 清空 ConfigManager 缓存
- 移除主进程和 IPC 事件监听器
- 清理非核心 Node.js 模块缓存
- 主动触发 V8 垃圾回收，释放堆内存
...
**日志管理器**
- 内存日志缓冲区从 1000 条压缩到 20 条
- 日志级别从 `info` 提升到 `error`，只记录关键错误
- 暂停日志文件写入，减少磁盘 I/O

**代理管理器**
- 健康检查间隔从 10 秒延长到 30 秒
- 日志文件监控间隔从 500ms 延长到 2 秒

**内存监控系统**（`src/main/utils/memory-monitor.ts`）
- 每 15 秒采样一次内存使用（轻量模式下阈值更严格）
- 堆内存超过阈值时自动触发 GC，冷却时间 30 秒
- 正常模式阈值：堆 100MB / RSS 200MB；轻量模式阈值：堆 50MB / RSS 100MB

**渲染进程**（窗口重新打开后生效）
- 实时日志组件最多保留 50 条日志，每 30 秒清理一次
- Zustand store 使用深度比较，避免无变化时触发重渲染
- IPC 请求增加 1 秒缓存和 100ms 节流，减少重复调用
...
|------|----------|----------|
| 主进程堆内存 | 基准 | 减少约 30–50% |
| 健康检查频率 | 每 10 秒 | 每 30 秒 |
| 日志文件写入 | 持续 | 暂停 |
| IPC handlers | 全部注册 | 全部注销 |
